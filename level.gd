...
func _physics_process(delta: float) -> void:
	if multiplayer.is_server()==false:
		...
		if player == null:
			return
		%RollbackFrames.text = "Rollback Frames: %d, ping: %d, server delta %d" \
		% [NetcodeManager.get_rollback_frames(), player.ping, NetcodeManager.last_processed_tick - NetcodeManager.last_unconfirmed_player_tick]
